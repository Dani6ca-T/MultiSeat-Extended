...
///
/// ⚠️ This table was decorative until this port (upstream 4544429): <see cref="CanTransitionTo"/>
/// was called from tests only, which asserted the table against itself, so nothing noticed that
/// the code performed transitions the table forbade — including <c>Error -> Ready</c>, the
/// deliberate recovery path of POST /api/seats/{id}/session-reconnect (PR #22),
/// <c>Ready -> Error</c> from the health check, and the same-state re-asserts of LaunchApp and
/// recovery. The table was wrong, not the code. Both are now reconciled and
...
public static class SeatState
{
    ...
    /// named. It is applied anyway: this table was never enforced, every discrepancy found when it
    /// was first checked was in the *table*, and refusing a transition on a deployed host would
    /// strand a real seat over a bookkeeping disagreement. Failing loudly in the log while keeping
    /// the seat moving is the safer half of the trade.
    ///
